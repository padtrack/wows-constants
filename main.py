...
GAME_PATH = r"E:\WoWs\World_of_Warships"
...
with open(DUMP_PATH) as fp:
    ...
    shutil.copy(
        DUMP_PATH,
        os.path.join(
            os.path.dirname(__file__), "data", "versions", f"{version['BUILD']}.json"
        ),
    )
    shutil.copy(
        DUMP_PATH, os.path.join(os.path.dirname(__file__), "data", f"latest.json")
    )
